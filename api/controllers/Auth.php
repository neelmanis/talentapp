   ...
   */
    ...
    $method = $this->input->method(TRUE);
      ...
      if($this->form_validation->run($this) == FALSE){
        ...
        $query = "SELECT * FROM registration WHERE LOWER(email)='$serchMail' ";
        $registration = $this->Mdl_api->customQuery($query);
        ...
        // }else if($registration[0]->type == "talent" && $registration[0]->is_mail_verified == "no"){
        }else if($registration[0]->is_mail_verified == "no"){
          json_output(200,array('status'=>'fail','message'=>'talent-verify-mail'));
        }else if($registration[0]->type == "talent" && $registration[0]->is_mail_verified == 'yes' && $registration[0]->application_status == 'pending'){
          json_output(200,array('status'=>'fail','message'=>'talent-profile-review'));
          ...
          if($is_valid_password){
            ...
            $isTalent = $registration[0]->is_talent == "yes" ? true : false;

            $registrationId = $registration[0]->registration_id;
            if($registration[0]->type == 'talent'){
              $get_details = "SELECT fullname_en, fullname_ar, profile_image, is_profile_updated FROM talent_details WHERE registration_id=$registrationId";
            ...
            if($user_info !== "NA"){
              if($registration[0]->type == 'talent'){
                ...
                $profile_update = $user_info[0]->is_profile_updated == "yes" ? true : false;
              }else{
            ...
            json_output(200,array('status'=>'success','user'=>$user,'profileStatus'=>$profile_update,'isApproved'=>$isApproved, 'isTalent'=>$isTalent));
          }else{
    ...
    $method = $this->input->method(TRUE);
      ...
      if($this->form_validation->run($this) == FALSE){
        ...
        if($registration == "NA"){
          ...
          json_output(200,array('status'=>'fail','message'=>'user-email-exist'));
        } 
      }
		}
  }
