...
class Talent extends Generic{
   ...
   */
	public function mobile_ind_check($mobile){
		if($mobile == ""){
			$this->form_validation->set_message('mobile_ind_check','talent-phoneNumber-required');
			return false;
   ...
   */
	public function mobile_intl_check($mobile){
		if($mobile == ""){
			$this->form_validation->set_message('mobile_intl_check','talent-phoneNumber-required');
			return false;
  ...
  private function checkTalentSlug($slug){
    ...
    if($slug == ""){
      ...
      while($find){
        if($this->Mdl_api->isExist('talent_details',array('slug'=>$new_slug))){
   ...
   */
    ...
    $method = $_SERVER['REQUEST_METHOD'];
      ...
      if($this->form_validation->run($this) == FALSE){
        ...
        $template = "talent email verification";
        $mail_data = array(
   ...
   */
    ...
    if($method !== 'POST'){
      ...
      $get_talent_details = "SELECT fullname_en, is_profile_updated FROM talent_details WHERE registration_id='$registrationId' ";
      ...
      if($registration == "NA" || $talent_details == "NA" || $registration[0]->type == 'user'){
      ...
      }else if($registration[0]->is_mail_verified == "no"){
        ...
        $template = "admin notification - talent email verified";
        $mail_data = array(
          // 'view_file' => 'admin/talent-mail-verified',
          // 'subject' => 'Talent Application Submission',
          'template' => $template,
          'lang' => 'en',
          'to' => 'support@halagram.me',
          'halagram_logo' => base_url()."assets/images/HalaLogo.png",
          'isAttachment' => false,
          "name" => $talent_details[0]->fullname_en,
          "profile_url" => base_url().'talents/details/'.$uid
        );
        Modules::run('email/template',$mail_data);  
        
        json_output(200,array('status'=>'success','message'=>'talent-email-verified'));
      }else if($registration[0]->is_mail_verified == "yes" && $registration[0]->application_status == "pending"){
        json_output(200,array('status'=>'success','message'=>'talent-email-verified'));
      }else{
  ...
  public function activation(){
    ...
    if($method !== 'POST'){
      ...
      if($registration == "NA" || $talent_details == "NA" || $registration[0]->type == 'user'){
        json_output(200,array('status'=>'danger','message'=>'something-went-wrong'));
      }else if( $talent_details[0]->is_profile_updated == "no" ){
        ...
        json_output(200,array('status'=>'success','message'=>'talent-acccount-activated','user'=>$user));
      }else if($registration[0]->account_status == "deactive"){
        json_output(200,array('status'=>'danger','message'=>'account-deactive'));
      }else{
        json_output(200,array('status'=>'activated','message'=>'talent-acccount-activated'));
      }
		}
  }
    ...
    if($method !== 'GET'){
      ...
      if($check_token_validity['status'] === "invalid"){
        ...
        if($talent_registration !== "NA" && $talent_details !== "NA"){
          if($talent_registration[0]->application_status === "rejected"){
            $response = array(
              "isRejected" => $talent_registration[0]->application_status === "rejected" ? true : false,
              "fullName" => $talent_details[0]->lang == "en" ? $talent_details[0]->fullname_en : $talent_details[0]->fullname_ar,
              "stageName" => $talent_details[0]->lang == "en" ? $talent_details[0]->stage_name_en : $talent_details[0]->stage_name_ar,
            ...
            $category_change_request = $this->Mdl_api->retrieve('talent_category_change_request',array('registration_id'=>$registration_id,'status'=>'pending'));
            
            $talent_bank_details = $this->Mdl_api->retrieve('talent_bank_details',array('registration_id'=>$registration_id));

            $response = array(
              "isRejected" => $talent_registration[0]->application_status === "rejected" ? true : false,
              "isApproved" => $talent_registration[0]->profile_status === "active" ? true : false,
              "allowCatChange" => $category_change_request == "NA" ? true : false,
            ...
            if($talent_category_master !== "NA"){
              ...
              foreach($talent_category_master as $category){
                $selCat = $this->Mdl_api->retrieve('category_master', array('category_id'=>$category->category_id));
                if($selCat !== "NA"){
                  $name = $activeLang == "en" ? $selCat[0]->category_name_en : $selCat[0]->category_name_ar; 
                  $categories[] = (object) array('label' => $name, 'value' => $category->category_id);
                }
                
              }
              $response['categories'] = $categories;
              $response['categoryRequest'] = $categories;
   ...
   */
    ...
    if($method !== 'GET'){
      ...
      if($check_token_validity['status'] === "invalid"){
        ...
        $registration_id = $check_token_validity['registration_id'];
        $get_intro_video = "SELECT intro_video FROM talent_details WHERE registration_id='$registration_id' ";
        $talent_details = $this->Mdl_api->customQuery($get_intro_video);
        
        if($talent_details !== "NA"){
          json_output(200, array('status' => 'success','introVideo' => $talent_details[0]->intro_video ));
        }else{
          json_output(200, array('status'=>'no data'));
        }
      }
      ...
      if($check_token_validity['status'] === "invalid"){
        ...
        if($_FILES['welcomeVideo']['name'] !== "" ){
          ...
          if(! in_array($ext, $video_allowed_type)){
            json_output(200, array('status'=>'error', 'errorData'=>array('welcomeVideo'=>'invalid-video-type')));
          }else if($_FILES['welcomeVideo']['size'] > 100000000){
            json_output(200, array('status'=>'error', 'errorData'=>array('welcomeVideo'=>'video-file-size')));
          }
        }else{
          json_output(200, array('status'=>'error', 'errorData'=>array('welcomeVideo'=>'intro-video-required')));
        }

        $videoname = 'raw-'.$uid.'.webm';
        $uploadPath = './assets/video/';
        $upload = $this->uploadFile($videoname,$uploadPath,'0','welcomeVideo');
        ...
        if($upload !== 1){
          ...
          $thumb_path = 'assets/video/thumb-'.strtotime("now").'.jpg';
          $new_name = 'video-'.strtotime('now').'.mp4';
          $key = $uid.'/'.$new_name;
            
          $this->addThumbnail($raw_video, $thumb_path, $key);
  ...
  */
   ...
   */
    ...
    $headers = $this->input->request_headers();
      ...
      if($check_token_validity['status'] === "invalid"){
        ...
        $talent_details_query = "SELECT talent_id, is_profile_updated, platform_commision, info_uploaded, intro_uploaded FROM talent_details WHERE registration_id='$registration_id' ";
        ...
        if($this->form_validation->run($this) == FALSE){
          ...
          if(!empty($custom_errors)){
            ...
            $triggerMail = false;
            $is_profile_updated = 'no';
            if($talent_details[0]->info_uploaded == 'no' && $talent_details[0]->intro_uploaded == 'yes'){
              $triggerMail = true;
              $is_profile_updated = 'yes';
            }else if($talent_details[0]->info_uploaded == 'yes' && $talent_details[0]->intro_uploaded == 'yes'){
              $is_profile_updated = 'yes';
            }
            ...
            if($triggerMail){
              $profile_link = base_url().'talents/details/'.$uid;

              $mail_data = array(
                // 'view_file' => 'admin/talent-profile-update',
                // 'subject' => 'Talent Profile Completion',
                'template' => 'admin notification - talent profile updated',
                'lang' => 'en',
                'to' => 'support@halagram.me',
                'halagram_logo' => base_url()."assets/images/HalaLogo.png",
                'isAttachment' => false,
            ...
            if($talent_details[0]->intro_uploaded == 'no'){
              $message = 'talent-profile-success';
              $text = 'record-welcome-video';
            }else if($talent_details[0]->is_profile_updated == 'no'){
              $message = 'talent-profile-success';
              $text = 'profile-verification';
            }else if($talent_registration[0]->profile_status == "pending"){
              $message = 'talent-profile-success';
              $text = 'profile-verification';
            }
      ...
      if($check_token_validity['status'] === "invalid"){
        ...
        if($this->form_validation->run($this) == FALSE){
          ...
          if(!empty($custom_errors)){
            ...
            $talent_data = array(
              "fullname_en" => $content['lang'] == "en" ? $content['fullName'] : "",
              "fullname_ar" => $content['lang'] == "ar" ? $content['fullName'] : "",
              "stage_name_en" => $content['lang'] == "en" ? $content['stageName'] : "",
              "stage_name_ar" => $content['lang'] == "ar" ? $content['stageName'] : "",
              "country" => strip_tags($content['country']),
              "phone_code" => strip_tags($content['phoneCode']),
              "phone" => strip_tags($content['phoneNumber']),
              "contact_preference" => strip_tags($content['contactPreference']),
            ...
            json_output(200,array('status'=>'success', 'message'=>'profile-updated', 'text'=>'')); 
          }
        }
      }
    }
   ...
   */
    ...
    $headers = $this->input->request_headers();
      ...
      if($check_token_validity['status'] === "invalid"){
        ...
        $talent_details_query = "SELECT talent_id, is_profile_updated, platform_commision, lang, fullname_en, fullname_ar, email FROM talent_details WHERE registration_id='$registration_id' ";
        $talent_details = $this->Mdl_api->customQuery($talent_details_query);
        ...
        if(isset($content['categoryRequest']) && empty($content['categoryRequest'])){
          $custom_errors['categoryRequest'] = 'select-category';
          json_output(200,array('status'=>'error','errorData'=>$custom_errors));
        }else{

          $prevCat = array();
          if($talent_category_master !== "NA"){
            foreach($talent_category_master as $cat){
              array_push($prevCat, $cat->category_id);
            }
          }

          $catArr = array();
          foreach($content['categoryRequest'] as $category){
            array_push($catArr, $category['value']);  
          }
          
          $catStr = implode(",",$catArr);
          $prevCat = implode(",",$prevCat);
          ...
          $insert = $this->Mdl_api->insert("talent_category_change_request",$request_data);

          $name = $subject = $lang = '';
          $lang = $talent_details[0]->lang;
          $name = $lang == "en" ? $talent_details[0]->fullname_en : $talent_details[0]->fullname_ar;

          // if($lang === "en"){
          //   $subject = "HalaGram - Category Change Request";
          // }else{
          //   $subject = "هلاجرام - طلب تحديث الفئة";
          // }
          // $email_view_file = 'category-change-request-'.$lang;
          
          $mail_data = array(
            // 'view_file' => 'talent/'.$email_view_file,
            // 'subject' => $subject,
            'template' => "talent category change request",
            'lang' => $lang,
          ...
          $admin_mail_data = array(
            // 'view_file' => 'admin/talent-category-change-request',
            // 'subject' => 'Category Change Request',
            'template' => "admin notification - talent category change request",
            'lang' => "en",
            'to' => 'support@halagram.me',
            'isAttachment' => false,
            'halagram_logo' => base_url()."assets/images/HalaLogo.png",
          ...
          Modules::run('email/template',$admin_mail_data);  

          json_output(200,array('status'=>'success','message'=>'change-request-sent', 'text'=>'change-request-text'));           
        }
      }
    }
  }
  ...
  public function search(){
    ...
    $headers = $this->input->request_headers();
      ...
      $GET_TALENTS = "SELECT t.talent_id, t.registration_id, t.fullname_en, t.fullname_ar, t.stage_name_en, t.stage_name_ar, t.slug, t.profile_image, t.halagram_price, t.display_order, r.application_status FROM talent_details t INNER JOIN registration r ON r.registration_id = t.registration_id WHERE ( r.application_status='approved' AND r.profile_status='active' AND r.is_talent='yes' ) AND ( LOWER(t.stage_name_en) LIKE LOWER('%$serach%') OR LOWER(t.stage_name_ar) LIKE LOWER('%$serach%') ) ORDER BY t.display_order ASC";
      $talents = $this->Mdl_api->customQuery($GET_TALENTS);
      
      if($talents !== "NA"){
        $response = array();
        foreach($talents as $val){
          // if($val->application_id == 'approved'){
            $temp = array();
            ...
            $temp['nameEn'] = $val->stage_name_en;
            $temp['nameAr'] = $val->stage_name_ar;
  ...
  public function wishlist(){
    ...
    if($method !== 'GET'){
      ...
      if($check_token_validity['status'] === "invalid"){
        ...
        $talent = $this->Mdl_api->getWishlist($lang, $filter, $registration_id, $uid, $type, $from, $records_per_page);
        $total_records = $this->Mdl_api->totalWishlistRecords($lang, $filter, $registration_id, $uid, $type);
   ...
   */
    ...
    $headers = $this->input->request_headers();
      ...
      $talent_details_query = "SELECT r.uid, t.talent_id, t.registration_id, t.fullname_en, t.fullname_ar, t.stage_name_en, t.stage_name_ar, t.about_talent_en, t.about_talent_ar, t.profile_image, t.intro_video, t.halagram_price FROM talent_details t INNER JOIN registration r ON r.registration_id = t.registration_id WHERE t.slug='$talent_slug' ";
      $talent_details = $this->Mdl_api->customQuery($talent_details_query);
      ...
      if($talent_details !== "NA"){
        ...
        $get_duration = "SELECT AVG(complition_duration) AS duration FROM request WHERE talent_id='$talentId' AND request_status='complete' ";
        $reqDuration = $this->Mdl_api->customQuery($get_duration);

        if($reqDuration == "NA" || is_null($reqDuration[0]->duration)){
          $duration = 5;
          $durationUnit = 'days';
        }else{
          $avg_duration = $reqDuration[0]->duration;
          $minutes = round($avg_duration/60);
          if($minutes < 60 ){
            $duration = $minutes;
        ...
        $get_halagram = "SELECT service_type, occasion, halagram_link, halagram_thumbnail FROM request WHERE talent_id='$talentId' AND share_halagram='yes' AND share_halagram_talent='yes' AND request_status='complete' ORDER BY created_date DESC LIMIT 8";
        ...
        if($halagramVideos !== "NA"){
          ...
          foreach($halagramVideos as $val){
            ...
            $temp['halagramThumb'] = $val->halagram_thumbnail;
            $response[] = $temp;
          }
        ...
        $profile = array(
          ...
          'stage_name_en' =>  $talent_details[0]->stage_name_en,
          'stage_name_ar' =>  $talent_details[0]->stage_name_ar,
  ...
  */
  public function halagram(){
    ...
    $headers = $this->input->request_headers();
      ...
      $talent_details_query = "SELECT t.talent_id, t.fullname_en, t.fullname_ar, t.stage_name_en, t.stage_name_ar FROM talent_details t INNER JOIN registration r ON r.registration_id = t.registration_id WHERE t.slug='$talent_slug' ";
      $talent_details = $this->Mdl_api->customQuery($talent_details_query);

      if($talent_details !== "NA"){
        $talentId = $talent_details[0]->talent_id;
        $get_halagram = "SELECT service_type, occasion, halagram_link, halagram_thumbnail FROM request WHERE talent_id='$talentId' AND share_halagram='yes' AND share_halagram_talent='yes' AND request_status='complete' ORDER BY created_date DESC";
        $halagramVideos = $this->Mdl_api->customQuery($get_halagram);
        ...
        $profile = array(
          'fullname_en' =>  $talent_details[0]->stage_name_en,
          'fullname_ar' =>  $talent_details[0]->stage_name_ar,
          'halagrams' => $halagrams
        );
  ...
  public function counts(){
    ...
    $headers = $this->input->request_headers();
      ...
      if($check_token_validity['status'] === "invalid"){
        ...
        if($registration !== "NA"){
          $isApproved = $registration[0]->is_talent == 'yes' ? true : false;
          ...
          $get_reviews = "SELECT review_id FROM talent_review WHERE talent_id='$talentId' AND is_read='no' ";
          $reviews = $this->Mdl_api->customQuery($get_reviews);

          $get_reaction = "SELECT request_id FROM request WHERE talent_id='$talentId' AND reaction_link!='' AND request_status='complete' AND reaction_read='no' ";
          $reactions = $this->Mdl_api->customQuery($get_reaction);
          ...
          if($reactions !== "NA"){
            $reactionCount = sizeof($reactions);
          }
        }
        
        json_output(200, array("status"=>"success", "requestCount"=>$requestCount, "reviewCount"=>$reviewCount, "reactionCount"=>$reactionCount, "isApproved"=>$isApproved ));       
      }
    }
  }
  ...
  public function counter(){
    ...
    $headers = $this->input->request_headers();
      ...
      if($check_token_validity['status'] === "invalid"){
        ...
        $GET_REQUESTS = "SELECT talent_share, request_status FROM request WHERE talent_id='$talentId' ";
        ...
        if($request !== "NA"){
          foreach($request as $val){
            $totalRequest += 1;
            if($val->request_status !== 'created'){
              $totalRequest += 1;  
            }
             
            if($val->request_status == 'complete'){
              // $totalRequest += 1;
              $completedRequest += 1;
  ...
  public function visitorData(){
    ...
    $headers = $this->input->request_headers();
      ...
      if($check_token_validity['status'] === "invalid"){
        ...
        $GET_COUNTRY_DATA = "SELECT country, COUNT(visitor_id) AS total FROM talent_profile_visits WHERE type IN ('user','talent') AND talent_id='$talentId' AND country IS NOT NULL ";
        $GET_TOTAL_COUNTRY_DATA = "SELECT COUNT(visitor_id) AS total FROM talent_profile_visits WHERE type IN ('user','talent') AND talent_id='$talentId' ";
        ...
        if($dateFrom !== ''){
          ...
          $GET_TOTAL_COUNTRY_DATA .= " AND created_date >= '$date_from' ";
        }
        ...
        if($dateTo !== ''){
          ...
          $GET_TOTAL_COUNTRY_DATA .= " AND created_date <= '$date_to' ";
        }
        ...
        $countryCount = $this->Mdl_api->customQuery($GET_TOTAL_COUNTRY_DATA);
        ...
        $countryTotal = 0;
        if($countryCount !== "NA"){
          $countryTotal = $countryCount[0]->total;
        }
  ...
  */
  ...
  public function testTimestamp(){
    ...
    // $response = $this->generateDateRange('','');
    // print_r($response);
  ...
  public function remove(){
    ...
    foreach($registration_ids as $talent_registration_id){
      $registrationDetails = $this->Mdl_api->retrieve("registration",array("registration_id"=>$talent_registration_id, "type" => "talent"));

      if($registrationDetails !== "NA"){
        $talent_details = $this->Mdl_api->retrieve("talent_details",array("registration_id"=>$talent_registration_id));

        if($talent_details !== "NA"){
          $talent_id = $talent_details[0]->talent_id;

          $delete_from_app_authentication = $this->Mdl_api->delete("app_authentication",array("registration_id"=>$talent_registration_id, "type" => "talent"));

          $delete_from_authentication = $this->Mdl_api->delete("authentication",array("registration_id"=>$talent_registration_id, "type" => "talent"));

          $delete_from_status_change_log = $this->Mdl_api->delete("status_change_log",array("registration_id"=>$talent_registration_id));

          $delete_from_talent_bank_details = $this->Mdl_api->delete("talent_bank_details",array("registration_id"=>$talent_registration_id));

          $delete_from_talent_category_change_request = $this->Mdl_api->delete("talent_category_change_request",array("registration_id"=>$talent_registration_id));

          $delete_from_talent_category_master = $this->Mdl_api->delete("talent_category_master",array("registration_id"=>$talent_registration_id));

          $delete_from_talent_social_media_master = $this->Mdl_api->delete("talent_social_media_master",array("registration_id"=>$talent_registration_id));

          $delete_from_talent_profile_visits_A = $this->Mdl_api->delete("talent_profile_visits",array("registration_id"=>$talent_registration_id, "type" => "talent"));

          $delete_from_talent_profile_visits_B = $this->Mdl_api->delete("talent_profile_visits",array("talent_id"=>$talent_id));

          $delete_from_wishlist_A = $this->Mdl_api->delete("wishlist",array("registration_id"=>$talent_registration_id, "type" => "talent"));

          $delete_from_wishlist_B = $this->Mdl_api->delete("wishlist",array("talent_id"=>$talent_id));

          $delete_from_talent_review = $this->Mdl_api->delete("talent_review",array("talent_id"=>$talent_id));

          $requestsA = $this->Mdl_api->retrieve("request",array("registration_id"=>$talent_registration_id, "type" => "talent"));
          if($requestsA !== "NA"){
            foreach($requestsA as $reqA){
              $delete_from_payment_details = $this->Mdl_api->delete("payment_details",array("order_id"=>$reqA->order_id));
            }

            $requestsA = $this->Mdl_api->delete("request",array("registration_id"=>$talent_registration_id, "type" => "talent"));
          }
          
          $requestsB = $this->Mdl_api->retrieve("request",array("talent_id"=>$talent_id));
          if($requestsB !== "NA"){
            foreach($requestsB as $reqB){
              $delete_from_payment_details = $this->Mdl_api->delete("payment_details",array("order_id"=>$reqB->order_id));
            }

            $requestsB = $this->Mdl_api->delete("request",array("talent_id"=>$talent_id));
          }

          $delete_from_talent_details = $this->Mdl_api->delete("talent_details",array("registration_id"=>$talent_registration_id));

          $delete_from_registration = $this->Mdl_api->delete("registration",array("registration_id"=>$talent_registration_id, "type" => "talent"));
        }
