...
class Mdl_api extends Mdl_generic {
	...
	function getTalentByFilter($lang, $filter, $categories, $start, $limit){
		$selectList = "r.uid, t.fullname_en, t.fullname_ar, t.stage_name_en, t.stage_name_ar, t.profile_image, t.slug, t.halagram_price, t.display_order";

		$this->db->distinct();
		$this->db->from('talent_details t')
						->join('registration r', 'r.registration_id = t.registration_id');

		if(! empty($categories)){
			$selectList .= ", c.display_order";
		...
		$this->db->where('r.profile_status','active');
		$this->db->where('r.is_talent','yes');

		$this->db->select($selectList);

		if(! empty($filter)){
			if($lang == 'ar'){
				$this->db->where_in("LEFT(t.stage_name_ar, 1)",$filter);
			}else{
				$this->db->where_in("LEFT(t.stage_name_en, 1)",$filter);
			}
		}
		
		if(! empty($categories)){
			$this->db->order_by('c.display_order', 'asc');
			$this->db->order_by('t.stage_name_en', 'asc');
		}else{
	...
	function getWishlist($lang, $filter, $registration_id, $uid, $type, $start, $limit){
		$this->db->select('r.uid, t.fullname_en, t.fullname_ar, t.stage_name_en, t.stage_name_ar, t.profile_image, t.slug, t.halagram_price')
						->from('talent_details t')
						->join('registration r', 'r.registration_id = t.registration_id')
	...
	function totalWishlistRecords($lang, $filter, $registration_id, $uid, $type){
		$this->db->select('t.talent_id')
						->from('talent_details t')
						->join('registration r', 'r.registration_id = t.registration_id')
						->join('wishlist w', 'w.talent_id = t.talent_id');
