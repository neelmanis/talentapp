...
class Banner extends Generic{
	...
	public function index(){
		...
		if($method !== 'GET'){
			...
			$banner_text_query = "SELECT * FROM banner_text WHERE status='active' ORDER BY created_date DESC";
			$banner_text = $this->Mdl_api->customQuery($banner_text_query);

			$banner_image_query = "SELECT * FROM banner_image WHERE status='active' ORDER BY created_date DESC";
			...
			if($banner_video !== "NA"){
				$bannerVideo = array(
					...
					'background_image' => $banner_video[0]->background_image
				);
			}
