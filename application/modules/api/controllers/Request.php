  ...
  public function all(){
    ...
    $headers = $this->input->request_headers();
      ...
      if($check_token_validity['status'] === "invalid"){
        ...
        $GET_REQUEST_QUERY = "SELECT r.order_id, r.type, r.registration_id, r.uid, r.service_type, r.request_from, r.occasion, r.created_date, r.expiry_date FROM request r INNER JOIN talent_details t ON t.talent_id = r.talent_id WHERE t.registration_id=$registration_id AND r.request_status='confirmed' AND r.payment_status='hold' AND r.expiry_date >= '$curr_date' ORDER BY created_date asc ";
  ...
  public function details(){
    ...
    $headers = $this->input->request_headers();
      ...
      if($check_token_validity['status'] === "invalid"){
        ...
        $GET_REQUEST_QUERY = "SELECT r.order_id, r.service_type, r.request_to, r.request_from, r.occasion, r.details, r.reference_link, r.share_halagram, r.created_date FROM request r INNER JOIN talent_details t ON t.talent_id = r.talent_id WHERE t.registration_id='$registration_id' AND r.order_id='$orderId' ";
        $request = $this->Mdl_api->customQuery($GET_REQUEST_QUERY);
        ...
        if($request !== "NA"){
          ...
          $response['requestDate'] = date("d/m/Y",strtotime($request[0]->created_date));
   ...
   public function uploadHalagram(){
    ...
    $headers = $this->input->request_headers();
      ...
      if($check_token_validity['status'] === "invalid"){
        ...
        if($request !== "NA"){
          $video_allowed_type = array('webm','WEBM');
          ...
          $filename = $_FILES['halagramVideo']['name'];
          $ext = pathinfo($filename, PATHINFO_EXTENSION);
          $videoname = 'video-'.strtotime('now').'.mp4';
          $key = $uid.'/'.$videoname;
          $upload_response = $this->addObject($key,'halagramVideo');
          ...
          if($upload_response['upload']){
            ...
            $user_details = $this->Mdl_api->retrieve('registration', array('registration_id'=>$request[0]->registration_id, 'uid'=>$request[0]->uid, 'type'=>$request[0]->type));

            if($user_details !== "NA"){
              $download_link = $this->global_variables['front_end_url'].'download/'.$orderId;
              $email_view_file = 'download';
              $mail_data = array(
                'view_file' => 'user/'.$email_view_file,
                'to' => strip_tags($user_details[0]->email),
                'cc' => '',
                'bcc' => '',
                'subject' => 'Download Your Halagram',
                'isAttachment' => false,
      ...
      if($check_token_validity['status'] === "invalid"){
        ...
        if($request !== "NA"){
          ...
          if($_FILES['reactionVideo']['name'] !== "" ){
            ...
            $ext = pathinfo($filename, PATHINFO_EXTENSION);
            if(! in_array($ext, $video_allowed_type)){
              json_output(200, array('status'=>'error', 'message'=>'file is not allowed'));
            }else if($_FILES['reactionVideo']['size'] > 300000000){
              json_output(200, array('status'=>'error', 'message'=>'file size should be less than 300 mb'));
            }
          }else{
            json_output(200, array('status'=>'error', 'message'=>'record video'));
          }  
          ...
          $videoname = 'video-'.strtotime('now').'.mp4';
          $key = $uid.'/'.$videoname;
          $upload_response = $this->addObject($key,'reactionVideo');
          ...
          if($upload_response['upload']){
            ...
            json_output(200, array('status'=>'error', 'message'=>'Failed to upload'));
          }
        }else{
          json_output(200, array('status'=>'fail'));
        }
  ...
  public function completed(){
    ...
    if($method !== 'GET'){
      ...
      if($check_token_validity['status'] === "invalid"){
        ...
        $GET_REQUEST_QUERY = "SELECT r.order_id, r.type, r.service_type, r.registration_id, r.uid, r.occasion, r.halagram_link, r.reaction_link, r.share_halagram, r.share_reaction, r.share_halagram_talent, r.halagram_price, r.platform_share, r.talent_share, r.talent_payment_status, r.created_date FROM request r INNER JOIN talent_details t ON t.talent_id = r.talent_id WHERE t.registration_id='$registration_id' AND r.request_status='complete' AND r.payment_status='captured' ORDER BY created_date desc";
        $request = $this->Mdl_api->customQuery($GET_REQUEST_QUERY);
  ...
  public function byFilter(){
    ...
    $headers = $this->input->request_headers();
      ...
      if($check_token_validity['status'] === "invalid"){
        ...
        $GET_REQUEST_QUERY = "SELECT r.order_id, r.type, r.service_type, r.registration_id, r.uid, r.occasion, r.halagram_link, r.reaction_link, r.share_halagram, r.share_halagram_talent, r.share_reaction, r.halagram_price, r.platform_share, r.talent_share, r.talent_payment_status, r.created_date FROM request r INNER JOIN talent_details t ON t.talent_id = r.talent_id WHERE t.registration_id='$registration_id' AND r.request_status='complete' AND r.payment_status='captured' ";
  ...
  public function download(){
    ...
    $headers = $this->input->request_headers();
      ...
      if($check_token_validity['status'] === "invalid"){
        ...
        $GET_REQUEST_QUERY = "SELECT halagram_link, reaction_link FROM request WHERE registration_id='$registration_id' AND type='$type' AND uid='$uid' AND order_id='$orderId' AND  request_status='complete' ";
        // $request = $this->Mdl_api->retrieve('request',array('registration_id'=>$registration_id, 'type'=>$type, 'uid'=>$uid, 'order_id'=>$orderId, 'request_status'=>'complete'));
        $request = $this->Mdl_api->customQuery($GET_REQUEST_QUERY);

        if($request !== "NA"){
          json_output(200, array("status"=>"success","halagramLink"=>$request[0]->halagram_link, "reactionLink"=>$request[0]->reaction_link ));
        }else{
          json_output(200, array("status"=>"no data"));
        }
      }
