	            ...
	            if($userExist ===TRUE){
                   ...
                   if($userCheck !="NA"){
                       ...
                       if($verifyOtp=='verified')
                       {
                            ...
                            {
                                $response=array(
                                "Result"=>array("mobile"=>$mobile_number,"name"=>$name,"profileImage"=>$profileImage,"Authtoken"=>$token,"uid"=>$userCheck[0]->uid,"user_id"=>$userCheck[0]->id),
      ...
      }else
	  {
       //print_r($custom_errors);exit;
        ...
        $data = array(		
				...
				"is_public" => strip_tags($content['is_public']),				
		...
		if($user_info!='NA')
		{
			...
			$talent_category_master = $this->Mdl_services->customQuery($get_talent_categories);
			 ...
			 if($talent_language_master !== "NA"){
			  ...
			  $follower_info = $this->Mdl_services->retrieve('follower_master',array('user_id'=>$user_info[0]->id,'is_following'=>'1'));
			  if($follower_info!='NA')
			  {
			  ...
			  $following_info = $this->Mdl_services->retrieve('follower_master',array('follower_id'=>$user_info[0]->id,'is_following'=>'1'));
			...
			$data[] = array(		
					...
					"is_public" => ($user_info[0]->is_public=='0')?"Private":"Public",					
			...
			if($story_infoQ!='NA')
			{
				...
				$story_info = $this->Mdl_services->customQuery($story_info);
					...
					{ 
						...
						$stories[$key]['duet_allow']=$str->duet_allow;
						$stories[$key]['is_public']=$str->is_public;
...
function get_comments_stories()
{
    ...
    $headers = $this->input->request_headers();
	  ...
	  else if($check_token_validity['status'] === "valid")
	  {
		...
		$comment_info = $this->Mdl_services->retrieve('user_story_comments',array('story_id'=>$content['story_id'],'is_spam'=>'0','status'=>'Active'));
	//	echo $this->db->last_query();exit;
		if($comment_info!='NA')
		{
			
				$comment_count=sizeof($comment_info);
				$comment=array();
				
				$story_info="Select * from user_story_comments  where story_id='".$content['story_id']."' and status='Active' and is_spam='0' order by added_date DESC limit $start_index,$limit_per_page";
				$story_info = $this->Mdl_services->customQuery($story_info);
				if($story_info!='NA')
				{	
					foreach($story_info as $key=>$str)
					{ 
						$userInfo="Select * from user_registration  where id=".$str->user_id." and account_status='1'";
						$userInfo = $this->Mdl_services->customQuery($userInfo);						
						if($userInfo!='NA') 
						{
							$comment[$key]['comment_id']=$str->id;
							$comment[$key]['comment']=$str->comment;
							$comment[$key]['parent_id']=$str->parent_id;
							$comment[$key]['added_date']=date("Y-m-d H:i:s",strtotime($str->added_date));
							$comment[$key]['full_name']=$userInfo[0]->full_name;
							$comment[$key]['uid']=$userInfo[0]->uid;
							$comment[$key]['username']=$userInfo[0]->username;
							$comment[$key]['profile_image']=$userInfo[0]->profile_image;
							
						}
						$commentLikeInfo="Select * from user_story_comments_like  where comment_id=".$str->id." and is_liked='1'";
						$commentLikeInfo = $this->Mdl_services->customQuery($commentLikeInfo);
						if($commentLikeInfo!='NA')
						{
							foreach($commentLikeInfo as $likeKey=>$strKey)
							{
								//echo "Key=" . $likeKey . ", Value=" . $strKey;
								//echo "<pre>"; print_r($commentLikeInfo);exit;
								$userInfoLC="Select * from user_registration  where id=".$strKey->user_id." and account_status='1'";
								$userInfoLC = $this->Mdl_services->customQuery($userInfoLC);							
								if($userInfoLC!='NA') 
								{
									$comment[$key]['commentsLike'][$likeKey]['liked_by_full_name']=$userInfoLC[0]->full_name;
									$comment[$key]['commentsLike'][$likeKey]['liked_by_uid']=$userInfoLC[0]->uid;
									$comment[$key]['commentsLike'][$likeKey]['liked_by_username']=$userInfoLC[0]->username;
									$comment[$key]['commentsLike'][$likeKey]['liked_by_profile_image']=$userInfoLC[0]->profile_image;
			...
			//$resultArray=array_merge($comment,$commentLikes);
					$response=array(
					"Result"=>$comment,
					"Message"=>"Data fetched succesfully",
					"status"=>true,
					"total_records"=>$comment_count);
				}
				else{
					$response=array(
					"Result"=>$comment,
...
function get_comments_from_commentId()
{
	$method = $_SERVER['REQUEST_METHOD'];
    $headers = $this->input->request_headers();
	  ...
	  else if($check_token_validity['status'] === "valid")
	  {
		...
		$comment_info = $this->Mdl_services->retrieve('user_story_comments',array('id'=>$content['comment_id']));
	//	echo $this->db->last_query();exit;
		if($comment_info!='NA')
		{
			$comment=array();	
			$commentLikes['commentsLike']=array();	
			
			
				
				
			foreach($comment_info as $key=>$str)
			{ 
				$userInfo="Select * from user_registration  where id=".$str->user_id." and account_status='1'";
				$userInfo = $this->Mdl_services->customQuery($userInfo);				
			...
			$commentLikeInfo="Select * from user_story_comments_like  where comment_id=".$content['comment_id']." and is_liked='1'";
			$commentLikeInfo = $this->Mdl_services->customQuery($commentLikeInfo);
			if($commentLikeInfo!='NA')
			{
				foreach($commentLikeInfo as $likeKey=>$strKey)
				{
					...
					if($userInfoLC!='NA') 
					{
						$commentLikes['commentsLike'][$likeKey]['full_name']=$userInfoLC[0]->full_name;
						$commentLikes['commentsLike'][$likeKey]['uid']=$userInfoLC[0]->uid;
						$commentLikes['commentsLike'][$likeKey]['username']=$userInfoLC[0]->username;
						$commentLikes['commentsLike'][$likeKey]['profile_image']=$userInfoLC[0]->profile_image;
 ...
 function follow_a_profile()
 {
    ...
    $headers = $this->input->request_headers();
	  ...
	  else if($check_token_validity['status'] === "valid")
	  {
		  $content = json_decode(file_get_contents('php://input'), TRUE);
	  //print_r($content);exit;
		$user_id=$content['user_id'];//eg: celebrity/actor
		$follower_id=$content['follower_id'];//current user
		$is_following=$content['is_following'];//0:unfollow 1:follow
		
		$ifFollowExist=$this->Mdl_services->isExist("follower_master", array('user_id'=>$user_id,'follower_id'=>$follower_id));
		if($ifFollowExist==false)
		{
                ...
                $insert_like = $this->Mdl_services->insert("follower_master",$follow_array);
				$response=array(
						"Result"=>'',
                ...
                $insert_like = $this->Mdl_services->update("follower_master",array('user_id'=>$user_id,'follower_id'=>$follower_id),$follow_array);
				$response=array(
						"Result"=>'',
 ...
 function story_views()
 {
    ...
    $headers = $this->input->request_headers();
	  ...
	  else if($check_token_validity['status'] === "valid")
	  {
		  $content = json_decode(file_get_contents('php://input'), TRUE);
	  //print_r($content);exit;
		$user_id=$content['user_id'];//eg: creator of video
		$viewer_id=$content['viewer_id'];//current user
		$story_id=$content['story_id'];
		
		$ifViewExist=$this->Mdl_services->isExist("user_story_views", array('user_id'=>$user_id,'story_id'=>$story_id,'ip_address'=>$_SERVER['REMOTE_ADDR']));
		if($ifViewExist==false)
		{
                $follow_array = array(
                  "user_id" => $user_id,                 
                  "viewer_id" => $viewer_id,
                ...
                $insert_like = $this->Mdl_services->insert("user_story_views",$follow_array);
				$response=array(
						"Result"=>'',
						"Message"=>"Viewer inserted.",
						"status"=>true
				);
        }
		else{
 ...
 function show_all_stories()
 {
    ...
    $headers = $this->input->request_headers();
		...
		$start_index = ($page - 1) * $limit_per_page;
			 ...
			 $story_infoQ="Select * from user_story  where status='Active' and content_warning='' order by added_date DESC";
			$story_infoQ = $this->Mdl_services->customQuery($story_infoQ);
			
			//echo "<pre>"; print_r($story_infoQ);
			//echo $this->db->last_query();exit;
			if($story_infoQ!='NA')
			{
				$story_count=sizeof($story_infoQ);
				$stories=array();
				$story_info="Select * from user_story  where  status='Active' and content_warning='' order by added_date DESC limit $start_index,$limit_per_page";
				$story_info = $this->Mdl_services->customQuery($story_info);
				//echo "<pre>"; print_r($story_info);
					if($story_info!='NA')
					{
					foreach($story_info as $key=>$str)
					{

						$user_info = $this->Mdl_services->retrieve('user_registration',array('id'=>$str->user_id));
